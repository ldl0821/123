...
$.widget("bz.grid", {
    ...
    _buildGrid: function(g) {
        ...
        $this.ds = new kendo.data.DataSource({
            ...
            change: function(e) {},
            error: function(e) {
                alert(lang.Bandex.Alert_1 + e.xhr.responseText);
            },
            requestEnd: function(e) {
                if (e.type == "update" || e.type == "destroy" || e.type == "create") {
                    if (e.response.IsSuccess) {
                        //Dialog.Notify("操作成功" + e.type);
                        showTipsMsg(lang.Bandex.Success + e.type, 3000, "ok");
                    } else {
                        x5alert(String.format("<div class='red fa-2x'><i class='fa fa-bell'></i>{0} " + lang.Bandex.ERROR + "</div><p>{1}</p>", e.type, e.response.ResultDescription));
...
//LH 2018/10/11 排序
function LhsortData(data){
    var Lhdata = data.Data;
    var dataList = [];
    var dataGroupList = [];
    Lhdata.GetAllMachineList = Lhdata.GetAllMachineList.filter(function(item){ return item.MAC_NBR != null});
    for(var i = 0; i <Lhdata.GetAllMachineGroupList.length;i++ ){
        dataGroupList.push(Lhdata.GetAllMachineGroupList[i].GP_NAME);
    }

    for(var q = 0; q <dataGroupList.length;q++ ){
        for(var w = 0; w < Lhdata.GetAllMachineList.length;w++){
           if(dataGroupList[q] == Lhdata.GetAllMachineList[w].COMPANY_NAME){
                dataList.push(Lhdata.GetAllMachineList[w]);
            }
        }
    }
    dataList = LhContrastDifference(Lhdata.GetAllMachineGroupList,Lhdata.GetAllMachineList).concat(dataList);
    return dataList;
}
// Lh差集
function LhContrastDifference(x, y) {
    let lhArray = Object.assign([],y);
    for(let i = 0; i < x.length;i++){
        for(let q = 0; q < y.length;q++){
            if(x[i].GP_NAME === y[q].GP_NAME){
                delete lhArray[q];
            }
        }
    };
    lhArray = lhArray.filter(function(val){
        return !(!val || val === "");
    });
    return lhArray;
}
        ...
        for (var j = 0; j < data2.length; j++) {
            if (data1[i].flag == "0") { //如果是账号组
                if (data1[i].id == data2[j].GP_NBR) {
                    ...
                    dd.attribute = data2[j].CATEGORY + "-" + data2[j].MAC_NAME + "-" +data2[j].MAC_NO;
                    // dd.text = data2[j].MAC_NAME;
                    dd.text = data2[j].MAC_NAME + "-" + data2[j].MAC_NO; //改为显示设备编号 htc:20180711
                    dd.icon = icon //"icon-user";
                    dd.expanded = true;
                    dd.flag = 1; //账号标志
                    data1[i].items.push(dd);
    ...
    $.widget("BZ.multipleComboxTree", {
        ...
        _create: function() {
            ...
            var contextstrhtml = '<div id="Combox_orgnizetree_' + eleid + '" class="Combox_orgnizetree" style="background-color:#FFFFFF; width: ' + (self.options.width + self.options.diffwidth) + 'px;height:' + self.options.cheight + 'px;display:none;">' +
                ...
                '<span style="margin-right: 10px;"><input type="checkbox" id="allcheck_' + eleid + '">' + lang.YieldProcessingBeat.All + '</span></div></div>' +
                '</div>';
            ...
            if (self.options.remote) {
                ...
                if (self.options.data.rData != undefined) { //判断有没有选择设备
                    $.ajax({
                        ...
                        success: function(data) {
                            var ndata = [];
                            $.each(parameter["DEFAULT"], function(item, k) {
                                if (k.EN_ABLE && k.HIS) {
                                    var tjson = {
                                        VALUE: item,
    ...
    $.widget("BZ.comboxList", {
        ...
        _create: function() {
            ...
            strhtml = '<div class="input-append">' +
                ...
                '<li class="all"><a data-title="Sent" href="javascript:;" class="btn">' + lang.Bandex.AllWordOrder + '</a><b></b></li>' +
                '<li class="ending active"><a data-title="Draft" href="javascript:;" class="btn">' + lang.Bandex.END + '</a><b></b></li>' +
                '<li class="working"><a data-title="Draft" href="javascript:;" class="btn">' + lang.Bandex.HomeWork + '</a><b></b></li>' +
                '<li class="nostart"><a data-title="Trash" href="javascript:;" class="btn">' + lang.Bandex.NoStart + '</a><b></b></li>' +
                '</ul>' +
                '</div>' +
                '</div>'
