    ...
    var ms = {
            ...
            return (function() {
                ...
                let AllTable = `<table class="table" border height="334" style="width: 100%;">
						    ...
						    <tr>
                                ...
                                <th style="background: #a6a6a6;">` + lang.Order.DeviceName + `</th>
                                <th style="background: #a6a6a6;">` + lang.Common.EquipmentModel + `</th>
                                <th style="background: #a6a6a6;">` + lang.Alarm.MachineNo + `</th>
						    </tr>
						    </thead>
        ...
        createDataTable: function(totalsubpageTmep, args) {
            return (function() {
                ...
                let dataTable = `<table class="table table1" border style="min-width: 100%;">
					    <thead>
					    <tr>
                ...
                option = {
                    ...
                    series: [{
                            ...
                            label: {
                                ...
                                // formatter: '{c}%'
                                formatter: (a) => {
                                    // let momentTime = moment(a.name).add(1, 'd').format('YYYY-MM-DD');
                                    // var m1 = moment(a.name),
                                    //     m2 = moment(momentTime);
                                    // if ($(window).width() < 1300) {
                                    //     return (Number(a.value)*m2.diff(m1,'second')/360000).toFixed(2)+'h' +'\n'+ Number(a.value).toFixed(0) + '%';
        ...
        calcData:function(data){
            let dataRaw = [];
            dataRaw[0] = +(Number(data.sumtime)*3600/(100-Number(data.data[5]))*data.data[0]).toFixed(0);
            dataRaw[1] = +(Number(data.sumtime)*3600/(100-Number(data.data[5]))*data.data[1]).toFixed(0);
            dataRaw[2] = +(Number(data.sumtime)*3600/(100-Number(data.data[5]))*data.data[2]).toFixed(0);
            dataRaw[3] = +(Number(data.sumtime)*3600/(100-Number(data.data[5]))*data.data[3]).toFixed(0);
            dataRaw[4] = +(Number(data.sumtime)*3600/(100-Number(data.data[5]))*data.data[4]).toFixed(0);
            dataRaw[5] = +(Number(data.sumtime)*3600/(100-Number(data.data[5]))*data.data[5]).toFixed(0);
            return dataRaw;
        }
    }
