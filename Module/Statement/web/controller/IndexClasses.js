    ...
    var ms = {
        ...
        createDataTable: function(totalsubpageTmep, args) {
            return (function() {
                ...
                let dataTable = `<table class="table table2" border style="min-width: 100%;">
					    <thead>
					    <tr>
                ...
                for (let i = 0; i < args.facility.value.length; i++) {
                    //添加时间
                    $(elementDataTable.getElementsByTagName('thead')[0].getElementsByTagName('tr')[0]).append(`<th colspan='${args.facility.value[i].length}' style="text-align: center">${args.facility.value[i][0].time} ${args.facility.value[i][0].SHIFT_NAME}</th>`)
                        //添加班次
        ...
        createEchart: function(totalsubpageTmep, args) {
            ...
            for (let i = 0; i < args.facility.value.length; i++) {
                for (let w = 0; w < args.facility.value[i].length; w++) {
                    ...
                    operationDataSum.push(args.facility.value[i][w].dataSum[1]);
                    leisureDataSum.push(args.facility.value[i][w].dataSum[2]);
                    debugDataSum.push(args.facility.value[i][w].dataSum[3]);
                    haltDataSum.push(args.facility.value[i][w].dataSum[4]);
                    shutdownDataSum.push(args.facility.value[i][w].dataSum[5]);
            ...
            function countArr(a){
                return (Math.max.apply(Array,a) + 2).toFixed(0);
            }
                    ...
                    yAxis: [{
                        ...
                        },
                        {
                            ...
                            axisLabel: {
                                ...
                                //     return formattingTimes(sumTime[index]*3600);
                                // }
