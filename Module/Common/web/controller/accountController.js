...
app.controller('accountctrl', ['$scope', '$http', function($scope, $http) {
    ...
    cols.push({ field: "REF_RESOURCE", title: lang.Common.AssociateName, width: 80, sortable: true, filterable: false, hidden: true });
    cols.push({ field: "COMPANY_NAME", title: lang.Common.AssociateCustomer, width: 80, sortable: true, filterable: false });
    ...
    $scope.adduser = function() {
        ...
        if (selectedNode.length > 0) { //判断是否有选中的节点
            ...
            COMBOX = $("#COMBOX").kendoDropDownList({
                ...
                optionLabel: {
                    COMPANY_NAME: lang.Maintain.PleaseSelect,
                    COMPANY_NBR: ''
                }
...
function f_edit(e) {
    ...
    COMBOX = $("#COMBOX").kendoDropDownList({
        ...
        optionLabel: {
            ...
            COMPANY_NBR: dataItem.REF_RESOURCE
        }
    }).data("kendoDropDownList");
...
function getDataByKeyWord() { //关键字查询
    ...
    $.post("/account/GetUserPageKey", data, function(data) {
        if (data.Status == 0) {
            var dd = [];
            for (var i = 0; i < data.Data.List.length; i++) {
                data.Data.List[i].EXPIRED = data.Data.List[i].EXPIRED == null ? "" : moment(data.Data.List[i].EXPIRED).format("YYYY-MM-DD HH:mm:ss");
            }
