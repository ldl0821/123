...
$(function() {
    ...
    cols.push({ field: "VALID_TIME", title: lang.Common.ValidTime, width: 80, sortable: true, filterable: false, template: '#:VALID_TIME#h' });
    //Grid
    grid = $("#grid").grid({
        //checkBoxColumn: true,
        ...
        actionUrl: ["getShiftSolutionList", "", "", "DelList"], //读、新增、更改、删除的URLaction
        custom: {
            ...
            $.ajax({
                ...
                success: function(data) {
                    var aaa = $("#MAC_NBRname").kendoComboBox({
                        ...
                        dataValueField: "COMPANY_NBR",
                        dataSource: data.Data,
                        value: data.Data[0].COMPANY_NBR,
                    }).data("MAC_NBRname");
                },
                error: function() {
    ...
    if (dataItem == undefined) { //新增
        ...
        for (var i = 0; i < dataItem.length; i++) {
            var tjson = {
                ...
                SHIFT_NBR: dataItem[i].SHIFT_NBR,
    ...
    viewModel = ko.mapping.fromJS({
        ...
        save: function(e) {
            ...
            if (validator.form()) {
                ...
                for (var i = 0; i < e.SHIFTS().length; i++) {
                    var tjson = {};
                    tjson.SHIFT_NBR = e.SHIFTS()[i].SHIFT_NBR();
                    tjson.SHIFT_NAME = e.SHIFTS()[i].SHIFT_NAME();
                    tjson.START_DATE = e.SHIFTS()[i].START_DATE();
                    tjson.RANK_NUM = i;
                    tjson.VALID_TIME = 0.00;
                    tjson.END_DATE = e.SHIFTS()[i].END_DATE();
                    shifts.push(tjson);
                }
                if (dataItem == undefined) { //新增
                    var dataA = {
                        company_nbr: parseInt($("#MAC_NBRname").val()), //绑定客户id htc:20180711
                        solution_name: e.SOLUTION_NAME(),
                        Li_shift: JSON.stringify(shifts)
                    }
                    $.post("/Shift/AddSolutionShift", dataA, function(data) {
                        if (data.Status == 0) {
                    ...
                    var dataU = {
                        solution_nbr: e.SOLUTION_NBR(),
                        soultion_name: e.SOLUTION_NAME(),
                        company_nbr: parseInt($("#MAC_NBRname").val()), //绑定客户id htc:20180711
                        Li_shift: JSON.stringify(shifts)
                    }
                    $.post("/Shift/ModifySolutionShift", dataU, function(data) {
                        if (data.Status == 0) {
                            $("#x5window").data("kendoWindow").close();
                            refreshGrid();
                            BzSuccess(data.Message);
...
function toolbar_editSolution(e) {
    ...
    //编辑时隐藏新增和删除 htc:20180712
    $(".delShiftBtn").each(function() { $(this).attr("style", "display:none;") });
    $("#addShiftBtn").attr("style", "display:none;");
    ...
    $.ajax({
        ...
        success: function(data) {
            var aaa = $("#MAC_NBRname").kendoComboBox({
                ...
                dataValueField: "COMPANY_NBR",
                dataSource: data.Data,
                value: SOLUTION_OBJ[0].COMPANY_NBR,
            }).data("MAC_NBRname");
        },
        error: function() {
            console.error(lang.Common.Error);
    ...
    $("#MAC_NBRname").val(SOLUTION_OBJ[0].COMPANY_NBR);
    $("input[name='MAC_NBRname_input']").val(SOLUTION_OBJ[0].CUSTOMER.split("|")[0]);
}
