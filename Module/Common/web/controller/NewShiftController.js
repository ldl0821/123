...
$(function() {
    ...
    function GetAllMachineAndMachineGroup1(id, chtml, callback) {
        ...
        //main/getUserMachineList  shift/GetAllMachineAndMachineGroup
        $.get("/shift/GetRecursionMacInGroupByUserId", { groupID: id }, function(data) {
            if (data.Status == 0) {
                data.Data.GetAllMachineList = data.Data.GetAllMachineList.filter(function(item){ return item.MAC_NBR != null});
...
function getPlanByMachine(e) {
    if (e != undefined) {
        ...
        var name = $(e).find('[attr="treenode"]').text().split("-")[$(e).find('[attr="treenode"]').text().split("-").length - 1];
                    ...
                    if (data.Data[i].solutionlist[0].CYCLE_TYPE == 0) {
                        ...
                        for (var j = 0; j < data.Data[i].solutionlist.length; j++) {
                            ...
                            for (var m = 0; m < data.Data[i].solutionlist[j].li.length; m++) {
                                ...
                                shtml = shtml + '<td>' + (data.Data[i].solutionlist[j].li[m].VALID_TIME) + lang.EmployeePerformance.Hours + '</td></tr>';
                            }
...
function onChange() {
    //获取seleced值
    var SOLUTIONS = $("#SOLUTIONS").data("kendoDropDownList").dataItems();

    //筛选出已选择的班次方案数据 htc:20180711
    var selectedItem = $("#SOLUTIONS").data("kendoDropDownList").value();
    var selectedData = _.where(SOLUTIONS, { SOLUTION_NBR: parseInt(selectedItem) });
        ...
        for (var i = 0; i <= selectedData.length; i++) {
            ...
            for (var j = 0; j < 9; j++) {
                if (i == 0) {
                    ...
                    if (j == 0) {
                        shtml = shtml + '<td><label class="checkbox"><input class="solutionCheckbox" type="checkbox" id="solutions' + selectedData[i - 1].SOLUTION_NBR + '" value="" style="margin-bottom: 0px;"/></label></td>';
                    } else if (j == 1) {
                        shtml = shtml + '<td>' + selectedData[i - 1].SOLUTION_NAME + '</td>';
                    } else {
                        shtml = shtml + '<td><label class="radio"><input checked class="solutions solutions' + selectedData[i - 1].SOLUTION_NBR + '" style="margin-bottom: 0px;" type="radio" name="Week_' + (j - 1) + '" value="' + (j - 1) + '" sid="' + SOLUTIONS[i - 1].SOLUTION_NBR + '"/></label></td>';
                    }
                }
            }
            shtml = shtml + '</tr>';
...
function deleteplan(e) {
    BzConfirm(lang.Order.DeleteData, function(d) {
        if (d) {
            var treeobj = $("#orgnizetree").data("kendoTreeView");
...
function addOrEdit(id, shifttype, attr_guid) {
    ...
    $("#SOLUTIONS").kendoDropDownList({
        ...
        dataSource: {
            transport: {
                read: {
                    dataType: "json",
                    url: "/Shift/getShiftSolutionList"
                }
            },
            schema: {
                data: function(response) {
                    if(response.Status == -1){
                        BzAlert(lang.Common.NoSchemePermissions);
                        return;
                    }
                    var tjson = {};
    ...
    if (id != undefined) { //编辑
        ...
        $("#SOLUTIONS").data("kendoDropDownList").value(sloutionID);
    ...
    $("#Add").bind("click", { ID: id, attr_guid: attr_guid }, function(event) {
        ...
        //时间验证 htc:20180717
        // if ($("#START_DATE").data("kendoDatePicker").value() != null && $("#END_DATE").data("kendoDatePicker").value() != null) {
        //     //结束时间大于等于当前时间并且结束时间大于开始时间
        //     if (($("#END_DATE").data("kendoDatePicker").value() > new Date(new Date().getTime() - 1 * 24 * 60 * 60 * 1000)) && ($("#END_DATE").data("kendoDatePicker").value() > $("#START_DATE").data("kendoDatePicker").value())) {

        //     } else {
        //         BzAlert("结束时间不能小于当前时间且需大于当前时间！");
        //         return;
        //     }
        // } else {
        //     BzAlert("请输入正确的日期！");
        ...
        if(moment($("#START_DATE").data("kendoDatePicker").value()).isBefore(moment(new Date()))){
            $("#START_DATE").parent().addClass("error");
            $("#SHIFT_ERROR").html(lang.Common.StartTimeShouldNotGreaterThanDay);
            return;
        }
        ...
        for (var i = 0; i < selectedData.length; i++) {
            ...
            tjson.SOLUTION_NBR = selectedData[i].SOLUTION_NBR;
        ...
        data.machineId = JSON.stringify(data.machineId);
        data.machineshiftlist = JSON.stringify(data.machineshiftlist);

        //邬峰版新增班次排班 htc:20180704
        var dataWF = {
            MARS: _.uniq(JSON.parse(data.machineId)).join(','),
            BEGIN_DATE: $("#START_DATE").val(),
            END_DATE: $("#END_DATE").val(),
            CYCLE_TYPE: parseInt($('input[name="CYCLE_TYPE"]:checked').val()),
            SOLUTION_NBR: parseInt($("#SOLUTIONS").val()),
            DAY_WEEK: day_weekWF
        }
        if (event.data.ID == undefined) { //新增
            $.post("/Shift/AddShiftWF", dataWF, function(data) {
                if (data.Data >= 0) {
    ...
    $("#machineTree").Tree({
        url: "/shift/GetRecursionMacInGroupByUserId",
        checkbox: true,
        type: 2,
