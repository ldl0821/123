    ...
    .controller('appCtrl', function($scope) {
        ...
        $.post(baseUrl, JSON.parse(par), function(rdata) {
            if (rdata.Status == 0) {
                ...
                // }
                var data = [];
                rdata.Data.forEach(function(v, i) {
                    switch (jQuery.parseJSON(par).date_type) {
                        case 1:
                            data.push({
                                "NO": i + 1,
                                "MAC_NAME": v.MAC_NAME || v.GP_NAME,
                                "TYPE": lang.EmployeePerformance.Shift,
                                "DATE": moment(v.NATURE_DAY).format('YYYY-MM-DD'),
                                "VALUE": (v.ACTIVATION_RATE * 100).toFixed(1),
                                "SHIFT": v.SHIFT_NAME
                            })
                            break;
                        case 2:
                            data.push({
                                "NO": i + 1,
                                "MAC_NAME": v.MAC_NAME || v.GP_NAME,
                                "TYPE": lang.EmployeePerformance.Day,
                                "DATE": moment(v.NATURE_DAY).format('YYYY-MM-DD'),
                                "VALUE": (v.ACTIVATION_RATE * 100).toFixed(1)
                            })
                            break;
                        case 3:
                            data.push({
                                "NO": i + 1,
                                "MAC_NAME": v.MAC_NAME || v.GP_NAME,
                                "TYPE": lang.EmployeePerformance.Weeks,
                                "DATE": v.NATURE_DAY,
                                "VALUE": (v.ACTIVATION_RATE * 100).toFixed(1)
                            })
                            break;
                        case 4:
                            data.push({
                                "NO": i + 1,
                                "MAC_NAME": v.MAC_NAME || v.GP_NAME,
                                "TYPE": lang.EmployeePerformance.Month,
                                "DATE": v.NATURE_DAY,
                                "VALUE": (v.ACTIVATION_RATE * 100).toFixed(1)
                            })
                            break;
                        case 5:
                            data.push({
                                "NO": i + 1,
                                "MAC_NAME": v.MAC_NAME || v.GP_NAME,
                                "TYPE": lang.EmployeePerformance.Years,
                                "DATE": v.NATURE_DAY,
                                "VALUE": (v.ACTIVATION_RATE * 100).toFixed(1)
                            })
                            break;
