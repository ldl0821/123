...
exports.activationpage = function(req, res) {
    post_common.permission(req, res, '/machineactivation', 'view', path.resolve(__dirname, '../../web/view/machineactivation/new_index'));
    // if (!req.session.user)
    //     res.redirect('/');
    // else
...
exports.OutPutIndex = function(req, res) {
    res.render(path.resolve(__dirname, '../../web/view/machineactivation/new_index_detail'), {
        menulist: req.session.menu,
        user: req.session.user,
        lang: post_common.getLanguage()
    });
